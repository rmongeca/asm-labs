...
mlog <- lm(log10(gross) ~ log10(budget) + duration + castfl + genre + yearcat +
             budget:yearcat + duration:genre + actor1fl:genre + castfl:genre,
           data = imdb)
glance(mlog)
op<-par(mfrow=c(2,2))
plot(mlog)
...
Having mentioned that, we decide to keep the previous model for the
interpretation part as it has a better fit with our data and it has a more
