...
We are interested in predicting the gross of a movie basic on its characteristics.
...
Using the basicStats we obtain the excess kurtosis, $K(X)-3$ and we see that we have 
a considerable positive one and that it has a right skewed distribution. So, it is not normal.
We should consider that the skewness and kurtosis could be due to outliers.

In the following graph we see the relation between $gross$ and all the numerical 
variables.
```{r}
imdb %>% 
...
We observe more of a linear relation between the pairs of gross and budget as well
as with duration. We can't discern any pattern between the pairs of gross and the 
Facebook variables: $directorfl,actor1fl,actor2fl,actor3fl,castfl$. 
The $actor3fl,directorfl$ could be separated in 2 clusters at the cutoff point of 5000 likes
and for the latter at the cutoff point of 10000 likes

We create a categorial variable ($yearcat$) with 3 levels: 2000-2005, 2006-2010 and 2011-2016
based on the $titleyear$ of the movie.
...
imdb %>% 
  ...
  ggpairs(.,
          title = "Imdb economical variables relation by Year genre", 
...
We observe two outliers in the Action genre based on their $gross$ value.
```{r}
...
```{r}
mc<-lm(gross~(.-genre-yearcat)*(genre+yearcat)+genre:yearcat,
       imdb%>% dplyr::select(-c(movietitle,titleyear)))

glance(mc)
```

Roughly 64% of the variance found in the response variable ($gross$) can be explained 
by the predictor variables. The obtained p-value ($omnibus test$) indicates that 
...
we can conclude that normality of the residuals is not met. From the $Residuals vs Fitted$ plot,
we seek to validate the assumption of homoskedasticity. We observe, a non 
random distribution of the points along the $y-axis$. All in all, we can't validate this model.

We observed slight curvature in the plotted line in the $Residuals vs Fitted$ plot. 
```{r}
car::residualPlots(mc, terms= ~ 1)
```

However, based on $p-value$ we can't reject the $H_0$ hypothesis and hence $\beta_2 =0$ so,
a model in which we only have a linear relationship is chosen. 

# Select significant variables

We use the stepwise procedure, by using the $BIC$ criterion, to select the significant variables.
We start with the complete model and eliminate in $both$ directions.
```{r}
summary(m1<-step(mc,direction="both",k=log(nrow(imdb))))
```


When dealing with categorical variables we should use the $Anova$ method and obtain the
variables kept using the stepwise procedure.
The $p-value$ obtained will allow us to say if the interaction variables are significant. 

```{r}
car::Anova(m1)
```

We see that neither $actor3fl$ nor $directorfl$ are included in the model and hence, 
we could consider categorizing these variables as it was mentioned in the exploratory 
analysis and see whether we obtain a better model. 
...
as well as $actor1fl$ and $actor2fl$. Consequently the coefficients can't be directly interpreted. 
```{r}
corr_mat=cor(imdb %>% 
              select_if(is.numeric) %>%
               select(-c(gross,titleyear)),method="s")
