...
ridge_lambda_search_cv <- function(
  ...
  for(i in 1:n) {
    ...
    for(j in 1:cv) {
      ...
      # Compute (X^TX + lambda*Id)^-1
      xx_inv <- v %*% d_inv %*% t(v)
      # Compute H such that y.valid.hat = H*y.train
      h <- x.valid %*% xx_inv %*% t(x.train)
      # Compute y prediciton y.hat
      y.hat <- h %*% y.train
      # Compute MSPE
      mspe <- sum((y.valid - y.hat)^2) / length(y.valid)
      # Compute Wsuch that y.train.hat = W*y.train
      w <- x.train %*% xx_inv %*% t(x.train)
      # Compute df
      df <- sum(diag(w))
      # Add results to output
      out.cv$mspe[j] <- mspe
...
(out.loocv <- ridge_lambda_search_loocv_gcv(x, y, lambda.v, plot = FALSE))
  (out.loocv2 <- ridge_lambda_search_cv(x, y, cv = nrow(x), lambda.v, plot = TRUE))
```
