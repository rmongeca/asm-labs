...
requireorinstall(c("sm","tidyverse","dplyr"))
```
...
script which contains different functions for bandwith selection.
We will choose the bandwith hyper-parameter by LOOCV. We use the appropiate 
...
In first place, we have to compute the local linear regression for the predicted
variable *lgWeight* depending on *Yr*. We choose the badnwith as the one that
...
After choosing the bandwith, we build the local linear regression model using
...
Now we have the residual values $\cap{\epsilon}_i = y_i - \cap{m}(x_i)$ and 
their logarithm $z_i = \log{\cap{\epsilon}^2_i }$. We need to build a new model 
for $z_i$ against $x_i$. We choose the new badnwith and build the model:
...
Finally, the conditional variance $\cap{\sigma}^2(x) =\exp{\cap{q}(x)}$ where 
$\cap{q}(x)$ is the estimate of the previous model.
...
superimposing the values of $\cap{\sigma}^2(x)$, and also the values of 
$\cap{m}(x)$ with the bands $\cap{m}(x) \pm 1.96\cap{\sigma}(x)$.
...
*Plug-in* bandwith parameter. Afterwards, we use this badnwith with the
*sm.regression* function in the *sm* package to compute the local linear 
regression models, both $\cap{m}(x)$ and $\cap{q}(x)$.
Regarding the Kernel choice, we decide to use the *normal* kernel.
